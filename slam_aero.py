...
# ----------------------- CONFIGURATION -----------------------
FCU_ADDR_r0 = 'tcp:127.0.0.1:5763'
FCU_ADDR_r1 = 'tcp:127.0.0.1:5773'
FCU_BAUD = 115200
HOME_LAT = 19.1345054
HOME_LON = 72.9120648
HOME_ALT = 53
...
class SlamLocalization(Node):
    def __init__(self, vehicle0, vehicle1):
        ...
        self.odom_subscription = self.create_subscription(Odometry, '/r0/odom', self.odom_callback0, qos)
        self.odom_subscription = self.create_subscription(Odometry, '/r1/odom', self.odom_callback1, qos)
        self.last_msg = None
        self.prev_pos = None
        self.prev_att = None
        self.prev_time = None
    ...
    def odom_callback0(self, msg):
        if msg is not None:
            ...
            if use_rangefinder == 1:
                rng_pos_z = -get_rangefinder_data(self.vehicle0)
            else:
            ...
            if compass_enabled == 1:
                self.initial_compass_yaw = math.radians(get_heading(self.vehicle0)) #yaw in radians 
            ...
            if self.prev_pos is not None:
                ...
                vision_speed_send(self.vehicle0, vx, vy, curr_vel[2])
                vision_position_send(self.vehicle0, pos_x, pos_y, rng_pos_z,curr_att[0], curr_att[1], curr_att[2])

                # Show debug messages here
                if debug_enable == 1:
                    subprocess.call('clear', shell=True) # This helps in displaying the messages to be more readable
                    print("Vehicle 0 (r0) Debug Info:")
    ...
    def odom_callback1(self, msg):
        if msg is not None:
            ...
            if use_rangefinder == 1:
                rng_pos_z = -get_rangefinder_data(self.vehicle1)
            else:
                rng_pos_z = V_aeroRef_aeroBody[2][3]
            ...
            rpy_rad = np.array( tf.euler_from_matrix(H_aeroRef_aeroBody, 'sxyz'))
            if compass_enabled == 1:
                self.initial_compass_yaw = math.radians(get_heading(self.vehicle1)) #yaw in radians 
                self.get_logger().info(f"Initial yaw offset (deg): {math.degrees(self.initial_compass_yaw)}")
            ...
            if self.prev_pos is not None:
                ...
                vision_speed_send(self.vehicle1, vx, vy, curr_vel[2])
                vision_position_send(self.vehicle1, pos_x, pos_y, rng_pos_z,curr_att[0], curr_att[1], curr_att[2])

                # Show debug messages here
                if debug_enable == 1:
                    subprocess.call('clear', shell=True) # This helps in displaying the messages to be more readable
                    print("Vehicle 1 (r1) Debug Info:")
                    progress("DEBUG: Raw RPY[deg]: {}".format( np.array( tf.euler_from_matrix( H_camRef_cambody, 'sxyz')) * 180 / math.pi))
                    progress("DEBUG: NED RPY[deg]: {}".format( np.array(rpy_rad) * 180 / math.pi))
                    progress("DEBUG: Raw pos xyz : {}".format( np.array( [position.x, position.y, position.z])))
                    progress("DEBUG: NED pos xyz : {}".format( np.array( tf.translation_from_matrix( H_aeroRef_aeroBody))))
...
def main(args=None):
    ...
    vehicle0 = connect_vehicle(FCU_ADDR_r0, FCU_BAUD)
    vehicle1 = connect_vehicle(FCU_ADDR_r1, FCU_BAUD)
    enable_data_stream(vehicle0, STREAM_RATE)
    enable_data_stream(vehicle1, STREAM_RATE)
    time.sleep(1)

    set_default_home_position(vehicle0, HOME_LAT, HOME_LON, HOME_ALT)
    set_default_home_position(vehicle1, HOME_LAT, HOME_LON, HOME_ALT)

    slam_node = SlamLocalization(vehicle0, vehicle1)
